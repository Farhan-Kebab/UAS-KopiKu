                ...
                <div class="flex items-center gap-x-10">
                    ...
                    <div class="hidden gap-x-10 sm:flex">
                        <a href="{{ route('filamentblog.post.index') }}" class="font-semibold text-md hover:text-primary-600">
                            <span>{{__('filament-blog::blog-views.components.header.blogs')}}</span>
                        </a>
                        <div class="relative group">
                            <button class="flex items-center justify-center font-semibold text-md hover:text-primary-600 gap-x-2">
                                <span>{{__('filament-blog::blog-views.components.header.categories')}}</span>
                    ...
                    <form action="{{ route('filamentblog.post.search') }}" method="GET">
                        <div class="relative">
                            <div class="relative">
                                ...
                                <input placeholder="{{__('filament-blog::blog-views.components.header.search')}}" type="text" name="query" value="{{ request()->get('query') }}" class="w-full px-6 py-3 pl-12 text-sm font-medium text-gray-800 placeholder-gray-400 border rounded-full outline-none bg-white/10 placeholder:text-slate-500 focus:ring-0" />
                            </div>
                            @error('query')
